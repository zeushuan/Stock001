...
## 8. v9.25.1 修正後重跑（AI_Storage_US 根因修正）

### 根因
- 原 AI_Storage_US 27 個訊號中，SNDK 當 leader 佔 22 個
- SNDK 2025-02 從 WDC 分拆 IPO，2025-11 仍在 post-IPO 高波動期
- 它的大漲多半是個股事件（解禁/財報），不是 NAND 族群催化劑
- 因此 MU/WDC/STX 沒跟漲 → 大量 SL

### 修正
```yaml
# config/peer_groups.yaml settings:
leader_min_history_days: 252  # 🆕 新 IPO 不能當 leader
```
...
|---|---|---|---|
| 訊號數 | 52 | 23 | -56% |
| 整體勝率 | 62.7% | **72.7%** | +10 pp |
| 平均報酬 | +2.54% | +2.93% | +0.4 pp |
| 0.75+ 勝率 | 67.6% | **85.7%** | +18 pp |
| 0.75+ 平均報酬 | +2.97% | +4.34% | +1.4 pp |

### By Group 對照

| Group | 修正前 win/n | 修正後 win/n |
|---|---|---|
| AI_Server_TW | 100% / 3 | 100% / 3 |
| AI_Power_US | 84.6% / 13 | **90.9% / 11** |
| AI_Chip_US | 50% / 4 | 50% / 4 |
| **AI_Storage_US** | 48.1% / 27 | **33.3% / 3** |
| TW_Semi_OSAT | 75% / 4 | 0% / 1 |

**AI_Storage_US 從 27 砍到 3** — 污染源 SNDK 被正確排除。
