...
| 策略 | 均值報酬 | 最大單股獲利 | 最大單股損失 | 風報比 | 跨年度 σ |
|------|---------|------------|------------|------|--------|
| ① 持有 BH | +163.94% | +8390% | -96% | 1.71 | — |
| ⑦ v7 base | +72.99% | +2441% | -140% | 0.52 | — |
| **v8 P0_T1T3** | **+197.48%** | +10299% | -290% | 0.68 | 12.7 |
| v8 P0_T1T3+CB30 | +134.07% | +3376% | -166% | 0.81 | 12.6 |
| **v8 P0_T1T3+POS** ★ 生產推薦 | **+141.68%** | +4027% | **-166%** | **0.85** | **8.9** |
...
```bash
# 單變體全市場（~12 秒）
python v8_runner.py --mode P0_T1T3+POS --workers 12     # ★ 生產推薦

# 多變體並行測試（~20 秒）
python batch_test.py --variants base,P0_T1T3+CB30,P0_T1T3+POS --workers-per 4

# 每日信號掃描（~3 秒）
python daily_scanner.py --top 30

# 深度分析
python feature_engineering.py     # 1243 檔特徵計算
python cluster_analysis.py        # Cohen's d 區辨力分析
python year_by_year.py             # 6 年逐年穩定性驗證
```
